...
//! `eIsSet`/`eUnset`.
//!
//! Design notes (matching the argument in `artop-common`):
//! - Object identity and references are `Rc<RefCell<dyn EObject>>`
//!   ([`artop_common::value::ObjectRef`]); feature *values* are the type-safe
//!   [`Val`] envelope.
...
//!   those integers.
//!
//! The generated AUTOSAR registry (`artop-metamodel`) and `artop-xmi` load a
...
pub use artop_common::value::{ObjectRef, Val};
