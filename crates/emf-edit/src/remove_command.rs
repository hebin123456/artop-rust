//! `RemoveCommand` — remove one or more values from a many-valued feature
//! (port of C++ `emf-edit` `RemoveCommand`, aligned to Java
//! `org.eclipse.emf.edit.command.RemoveCommand`).
//!
//! Undo/redo snapshots the pre-existing list on first execution, removes the
//! requested value(s) on execute and restores the prior list on undo. All
//! feature access goes through the [`emf_common::eobject::EObject`] reflection
//! surface, so the command works over any reflective object and is
//! domain-agnostic.
...
pub struct RemoveCommandRequest {
    /// The owner whose many-valued feature shrinks.
    pub owner: ObjectRef,
...
/// The mutable, undo/redo-capable `RemoveCommand`.
...
impl RemoveCommand {
    /// Build an executable [`RemoveCommand`].
    pub fn new(req: RemoveCommandRequest) -> Self {
        RemoveCommand {
    ...
    /// The post-state list with `remove` entries dropped (equality-based).
    fn build_removed(&self) -> Option<Val> {
        let st = self.state.borrow();
        let to_remove: Vec<Val> = match &st.value {
        ...
        list.retain(|item| !to_remove.iter().any(|r| r == item));
        Some(Val::List(list))
    }
}
...
impl Command for RemoveCommand {
    ...
    fn get_description(&self) -> String {
        "Removes a value from a feature".to_string()
    }
    fn dispose(&self) {}
...
/// Free-function factory mirroring EMF's static `RemoveCommand.create`.
pub fn remove(req: RemoveCommandRequest) -> CommandRef {
    std::rc::Rc::new(std::cell::RefCell::new(RemoveCommand::new(req)))
}
...
mod tests {
    ...
    #[test]
    fn remove_execute_undo_redo_via_stack() {
        let reg = registry();
        ...
        item.borrow_mut().e_set(
            "tags",
            Val::List(vec![Val::string("a"), Val::string("b"), Val::string("c")]),
        );
        ...
        assert_eq!(
            ...
            Some(Val::List(vec![Val::string("a"), Val::string("c")]))
        );
